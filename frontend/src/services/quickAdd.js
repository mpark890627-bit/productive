...
async function resolveTagId(tagName) {
    ...
    const exactMatch = existingTags.find((tag) => tag.name.trim().toLowerCase() === normalized.toLowerCase());
    if (exactMatch) {
...
export async function quickAddTask(rawInput) {
    const parsed = parseQuickAddInput(rawInput);
    const projectsPage = await getProjects({ page: 0, size: 200, sort: 'updatedAt,desc' });
    const project = projectsPage.content.find((item) => item.name.trim().toLowerCase() === parsed.projectName.trim().toLowerCase());
    if (!project) {
