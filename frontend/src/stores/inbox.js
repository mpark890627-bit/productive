...
const readStatusList = (value) => {
    if (!Array.isArray(value)) {
        return [];
    }
    return value.filter((item) => item === 'TODO' || item === 'IN_PROGRESS' || item === 'DONE');
};
...
export const useInboxStore = defineStore('inbox', () => {
    ...
    const watchedTaskIds = ref(new Set());
    const page = ref(0);
    const size = ref(20);
    ...
    const selectedSavedView = computed(() => savedViews.value.find((view) => view.id === selectedSavedViewId.value) ?? null);
    const isWatched = (taskId) => watchedTaskIds.value.has(taskId);
    const loadWatchState = async (items) => {
        ...
        const pairs = await Promise.all(items.map(async (task) => {
            try {
                ...
                return [task.id, watchers.some((watcher) => watcher.userId === currentUserId)];
            }
            catch {
    ...
    const setStatuses = async (statuses) => {
    ...
    const applySavedView = async (savedViewId) => {
    ...
    const removeSavedView = async (savedViewId) => {
