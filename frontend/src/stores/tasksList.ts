...
import { getTasksSummary, listTasksGlobal, type TasksSummary } from '../api/tasks'
...
export const useTasksListStore = defineStore('tasksList', () => {
  const filters = reactive<TasksFilters>({
    projectId: null,
    status: null,
    priority: null,
    assigneeUserId: null,
  ...
  const applyFilters = async (next: Partial<TasksFilters>) => {
    Object.assign(filters, next)
    page.value = 0
    await fetch()
  }

  const setKeywordDebounced = (keyword: string, delay = 300) => {
  ...
  const setSort = async (field: SortField, dir: SortDir) => {
