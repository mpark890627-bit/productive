    ...
    actions: {
        ...
        restore(options) {
            ...
            try {
                ...
                this.activeTabId = hasActive ? parsed.activeTabId : validTabs[0].id;
            }
            catch {
                this.tabs = [];
                this.activeTabId = '';
        ...
        buildTabId(route) {
            const strategy = route.meta.tabKeyStrategy ?? 'NAME_PARAMS';
        ...
        closeOthers(tabId) {
            const keep = new Set(this.tabs.filter((tab) => tab.affix || tab.id === tabId).map((tab) => tab.id));
