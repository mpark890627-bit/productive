...
import { createRisk, getRiskMatrixSummary, getRiskSummary, listRisks, type RiskCreatePayload } from '../api/risks'
import { useAuthStore } from './auth'
import type { RiskItem, RiskLevelBucket, RiskMatrixCell, RiskOwnerFilter, RiskStatus, RiskSummary } from '../types/risk'

interface RiskFilterState {
  status: RiskStatus | null
  levelBucket: RiskLevelBucket | null
  probability: number | null
  impact: number | null
  ownerFilter: RiskOwnerFilter
  keyword: string
  tagId: string | null
  sort: string
}

const defaultFilters = (): RiskFilterState => ({
  status: null,
  levelBucket: null,
  probability: null,
  impact: null,
...
export const useRiskStore = defineStore('risk', () => {
  ...
  const summaryCache = ref<Record<string, RiskSummary>>({})
  const matrix = ref<RiskMatrixCell[]>([])
  ...
  const filters = reactive<RiskFilterState>(defaultFilters())
  ...
  const addRisk = async (payload: RiskCreatePayload) => {
    if (!projectId.value) {
